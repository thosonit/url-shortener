...
|----|-------------|--------|
| FC003.1 | Search / filter | By code, URL, owner **email**, or status. (Owner is filtered by the email shown in results, not an internal id.) |
...
The two actions differ in what they disclose at redirect ([[FA002]]):
...
## Acceptance criteria
- Admins can locate links by multiple criteria, including owner email (FC003.1).
- Disabled links stop resolving at redirect with `404` ([[FA002]]) (FC003.2).
- Force-expire takes effect immediately, returning `410` at redirect (FC003.3).
